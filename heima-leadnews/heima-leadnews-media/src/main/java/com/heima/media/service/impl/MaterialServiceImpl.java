...
public class MaterialServiceImpl implements MaterialService {
    ...
    public ResponseResult uploadPicture(MultipartFile multipartFile) {
        ...
        try {
            ...
            log.error("user {} upload file {} to fastDFS error, error info:n",
                    user.getId(),originalFilename, e.getMessage());
        ...
        //设置返回值
        wmMaterial.setUrl(FILE_SERVER_URL + wmMaterial.getUrl());
    ...
    public ResponseResult delPicture(WmMaterialDto dto) {
        ...
        //防止服务器的地址写上去了，替换了就没有问题
        String fileId = wmMaterial.getUrl().replace(FILE_SERVER_URL, "");
        try {
    ...
    public ResponseResult findList(WmMaterialListDto dto) {
        ...
        datas = datas.stream().map((item) -> {
            item.setUrl(FILE_SERVER_URL + item.getUrl());
            return item;
