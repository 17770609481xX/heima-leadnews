...
@Setter
@Getter
@Configuration//这个注解加上@Bean，相当于生吃xml里面的bean容器
@ConfigurationProperties(prefix = "mysql.core")//前缀是这个，后面的东西会自动变化成去下划线，然后字母大写
@PropertySource("classpath:mysql-core-jdbc.properties")//加载数据库配置文件
@MapperScan(basePackages = "com.heima.model.mappers", sqlSessionFactoryRef = "mysqlCoreSqlSessionFactory")//mapper文件夹的目录，即dao层接口
